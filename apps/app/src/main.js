...
// ============================================================
// Camera — via PluginMessageHandler
// ============================================================
// The R1 camera is controlled by sending JSON messages through
// PluginMessageHandler.postMessage(). The Flutter bridge handles
// camera hardware and sends frames/captures back via onPluginMessage.
//
// Protocol:
//   Send: { "type": "camera", "action": "start" }   → activates camera preview
//   Send: { "type": "camera", "action": "capture" }  → captures current frame
//   Send: { "type": "camera", "action": "stop" }     → stops camera preview
//
// Receive via onPluginMessage:
//   { type: "cameraPreview", imageBase64: "..." }     → live preview frame
//   { type: "cameraCaptured", imageBase64: "..." }    → captured photo
...
function startCamera() {
  ...
  if (typeof PluginMessageHandler !== 'undefined') {
    ...
    showStatus('Camera: browser mode');
  }
}
  ...
  app.innerHTML = `
    <div class="camera-screen">
      <div class="camera-feed" id="cameraFeed"
        ...
        ${!state.cameraPreviewData ? '<div class="camera-waiting">📷 Waiting for camera…</div>' : ''}
      </div>
