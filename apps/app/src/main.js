...
window.onPluginMessage = async function(data) {
  ...
  // Skip sttStarted control messages
  if (data.type === 'sttStarted') { return; }
...
function sendToRabbit(message, noPrefix) {
  const fs = getActiveFolderSettings();
  if (!noPrefix) {
    const NO_SEARCH = '\uD83D\uDEA8 DO NOT search the internet. DO NOT save notes or create reminders. DO NOT create journal entries.';
    message = message.startsWith(NO_SEARCH) ? message : NO_SEARCH + '\n\n' + message;
  }
  if (typeof PluginMessageHandler !== 'undefined') {
    PluginMessageHandler.postMessage(JSON.stringify({
...
async function emailLogs() {
  ...
  if (typeof PluginMessageHandler !== 'undefined') {
    const prompt = '\uD83D\uDEA8 DO NOT SEARCH. Email this debug log to my Rabbit Hole:\n\n' + body;
    PluginMessageHandler.postMessage(JSON.stringify({ message: prompt, pluginId: 'com.r1.pixelart', useLLM: true, wantsR1Response: false, wantsJournalEntry: false }));
