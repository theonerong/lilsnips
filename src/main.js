// ============================================================
// Lil Snips — R1 Creation Plugin (Updated)
// ============================================================
...
let state = {
  ...
  cameraActive: false, cameraPreviewData: null,
  cameraFacing: 'back'  // 'back' = environment (Rabbit eye), 'front' = user
};
...
// ============================================================
// Camera — via PluginMessageHandler (Rabbit OS bridge)
// ============================================================
//
// Protocol (send):
//   { type: 'camera', action: 'start', facing: 'back' }   → start back camera (Rabbit eye)
//   { type: 'camera', action: 'start', facing: 'front' }  → start front camera (user-facing)
//   { type: 'camera', action: 'capture' }                  → capture current frame
//   { type: 'camera', action: 'stop' }                     → stop camera
//
// Protocol (receive via onPluginMessage):
//   { type: 'cameraPreview', imageBase64: '...' }          → live preview frame (updated ~30fps)
//   { type: 'cameraCaptured', imageBase64: '...' }         → captured photo returned
//   { type: 'cameraError', message: '...' }                → error occurred
//   { type: 'cameraStopped' }                              → camera stopped confirmation

let pttLocked = false;
let scrollLocked = false;   // 2-second lockout between front/back flips
const SCROLL_LOCK_MS = 2000;

function lockPtt(lock) {
  pttLocked = lock;
  if (typeof PluginMessageHandler !== 'undefined') {
    PluginMessageHandler.postMessage(JSON.stringify({ type: 'lockPtt', locked: lock }));
  }
}

function startCamera(facing = 'back') {
  ...
  if (typeof PluginMessageHandler !== 'undefined') {
    ...
    // Browser dev mode fallback — show placeholder
    state.cameraPreviewData = 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="240" height="240" viewBox="0 0 240 240"><rect fill="%23111" width="240" height="240"/><text x="120" y="120" text-anchor="middle" fill="%23666" font-size="12">Camera: browser mode</text></svg>');
    render();
    showStatus('Camera: browser dev mode');
  }
}
...
function captureCamera() {
  ...
  if (typeof PluginMessageHandler !== 'undefined') {
    ...
    // Browser dev mode — fake capture
    showStatus('Capture: not available in browser');
  }
}
...
// Switch between front and back camera while preview is active.
// Locked to once per 2 seconds to prevent rapid flipping.
function switchCamera() {
  if (!state.cameraActive || scrollLocked) return;

  // Lock for 2 seconds
  scrollLocked = true;
  setTimeout(() => { scrollLocked = false; }, SCROLL_LOCK_MS);

  const newFacing = state.cameraFacing === 'back' ? 'front' : 'back';
  state.cameraFacing = newFacing;
  ...
  if (typeof PluginMessageHandler !== 'undefined') {
    ...
    showStatus(newFacing === 'back' ? 'Switching to back…' : 'Switching to front…');
  } else {
    // Browser dev mode
    render();
    showStatus('Camera: switched to ' + newFacing + ' (dev mode)');
  }
}
...
// ============================================================
//
// Normal mode (camera OFF):
//   Single PTT → default context action (handled by handleSinglePtt)
//   Double PTT → activate back camera
//
// Camera mode (camera ON):
//   Single PTT → capture photo
//   Double PTT → stop camera and return home
//
// Double-click on R1 fires two sideClick events ~50ms apart.
...
function handleSideClick() {
  ...
  if (elapsed < DOUBLE_TAP_MS) {
    ...
    if (state.cameraActive) {
      // Camera ON + double PTT → stop and exit
      stopCamera();
      closeWebView.postMessage('');
    } else {
      // Camera OFF + double PTT → start back camera
      startCamera('back');
    }
    return;
  }
...
window.addEventListener('longPressStart', () => {
  if (state.cameraActive || pttLocked) return;
...
// Scroll Wheel
// ============================================================
// Camera ON + scroll → flip front ↔ back (2-second lockout)
// Camera OFF + scroll → normal list scrolling
...
window.onPluginMessage = async function(data) {
  // Camera preview frame (live feed from Rabbit OS)
  if (data.type === 'cameraPreview' && data.imageBase64) {
    if (state.cameraActive) {
      state.cameraPreviewData = 'data:image/jpeg;base64,' + data.imageBase64;
      const feed = $('#cameraFeed');
      if (feed) feed.style.backgroundImage = 'url(' + state.cameraPreviewData + ')';
    }
    return;
  }

  // Camera captured photo returned from OS
  if (data.type === 'cameraCaptured' && data.imageBase64) {
  ...
  // Camera error from OS
  if (data.type === 'cameraError') {
    state.cameraActive = false;
    state.cameraPreviewData = null;
    lockPtt(false);
    showStatus('Camera error: ' + (data.message || 'unknown'));
    render();
    return;
  }

  // Camera stopped confirmation
  if (data.type === 'cameraStopped') {
    state.cameraActive = false;
    state.cameraPreviewData = null;
    state.cameraFacing = 'back';
    lockPtt(false);
...
function renderCamera(app) {
  const facingLabel = state.cameraFacing === 'back' ? 'Back' : 'Front';
  app.innerHTML = `
    <div class="camera-screen">
      <div class="camera-feed" id="cameraFeed"
        ${state.cameraPreviewData ? 'style="background-image:url(' + state.cameraPreviewData + ')"' : ''}>
        ${!state.cameraPreviewData ? '<div class="camera-waiting">📷 Starting camera…</div>' : ''}
      </div>
      <div class="camera-hud">
        <div class="camera-hint">${facingLabel} • Scroll to flip • PTT: Capture • 2×PTT: Exit</div>
      </div>
    </div>`;
}
...
function emailFolder(folderId) {
  ...
  snips.forEach((n, i) => { body += 'Snip ' + pad(i + 1) + ':\n' + n.text + '\n\n'; });
